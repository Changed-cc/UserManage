...
public class Manage extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (users != null && users.size() > 0) {
            for (int i = 0; i < users.size(); i++) {
                ...
                out.println("<tr>"
                        ...
                        + "<td><a href='/UserManage/UserController?type=gotoUpdateUser&id=" + user.getId() + "'>修改</a>&nbsp;&nbsp;"
                        + "<a onclick='return delConfirm()' href='/UserManage/UserController?type=delUserById&id=" + user.getId() + "'>删除</a></td>"
                        + "</tr>");
            }
        }
        out.println("</table>");
        ...
        if (pageNow > 1) {
            ...
            out.println("<span id='begin' style='color:gray;'>首页</span>");
            out.println("<span id='prev' style='color:gray;'>上一页</span>");
        }
        ...
        if (pageNow < pageCount) {
            ...
            out.println("<span id='next' style='color:gray;'>下一页</span>");
            out.println("<span id='end' style='color:gray;'>尾页</span>");
        }
